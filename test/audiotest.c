...
main (int argc, char *argv[])
{
  ...
  g_object_set (G_OBJECT (src), "device", "plughw:1,0", NULL);
  audioconvert = gst_element_factory_make ("audioconvert", NULL);
  volume = gst_element_factory_make ("volume", "volume");
  g_object_set (G_OBJECT (volume), "volume", 5.0 , NULL);
  g_assert (audioconvert);

  spectrum = gst_element_factory_make ("spectrum", "spectrum");
  g_object_set (G_OBJECT (spectrum), "bands", spect_bands, "threshold", -50,
  ...
  caps = gst_caps_new_simple ("audio/x-raw-int",
      "rate", G_TYPE_INT, AUDIOFREQ, "channels", G_TYPE_INT, CHANNELS,  "depth", G_TYPE_INT, 16,NULL);
